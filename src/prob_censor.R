...
# Function to compute the probability of censoring based on various factors
prob_cens_dist <- function(min_day, max_day, cutoff, max_malaria, y_spline, mal_spline, HB_sigma, G_non_infect, grav_impact_df, grav_cats, shape_1_GA, shape_2_GA) {
  
  # Number of days
  n_days <- max_day - min_day + 1
  #print("ndays")
  #print(n_days)
  # Initialize placeholders for probabilities of being censored
  prob_censor_no_malaria <- numeric(grav_cats)
  prob_censor_malaria <- numeric(grav_cats)
  GA_beta_adj_censor <- array(0, dim = c(n_days, max_malaria + 1, grav_cats))
  
  # Compute probability of enrollment for each day
  prob_GA <- pbeta(((seq(min_day, max_day) + 1 - min_day) / n_days), shape1 = shape_1_GA, shape2 = shape_2_GA) -
    pbeta((seq(min_day, max_day) - min_day) / n_days, shape1 = shape_1_GA, shape2 = shape_2_GA)
  #prob_GA <- diff(pbeta((seq(min_day, max_day + 1) - min_day) / n_days, shape1 = shape_1_GA, shape2 = shape_2_GA))
  
  # Probability of being censored based on gestational age, gravidity, and malaria status
  no_malaria_probs_matrix <- t(outer(G_non_infect[1:grav_cats], seq_len(n_days), function(base, i) {
    pnorm(cutoff, mean = base + y_spline[i], sd = HB_sigma)
  }))
  malaria_probs_matrix <- mapply(function(base, effect) {
    sapply(seq_len(n_days), function(i) {
  ...
  for (i in seq_len(n_days)) {
    ...
    GA_beta_adj_censor[i, 1, ] <- (1 - no_malaria_probs_matrix[i, ]) * prob_GA[i]
    GA_beta_adj_censor[i, 2, ] <- (1 - malaria_probs_matrix[i, ]) * prob_GA[i]
  }
  
  # Normalize GA_beta_adj_censor
  for (j in 1:grav_cats) {
    GA_beta_adj_censor[, 1, j] <- GA_beta_adj_censor[, 1, j] / (1 - prob_censor_no_malaria[j])
    GA_beta_adj_censor[, 2, j] <- GA_beta_adj_censor[, 2, j] / (1 - prob_censor_malaria[j])
  }
  print(prob_censor_malaria)
  print(prob_censor_no_malaria)
  return(list(
    GA_beta_adj_censor = GA_beta_adj_censor,
    prob_censor_no_malaria = prob_censor_no_malaria,
    prob_censor_malaria = prob_censor_malaria
  ))
}