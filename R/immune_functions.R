...
get_weight_impact_grav_cat <- function(inf_history_matrix, index, imm_vect_sub) {
  # Sum the product of immunity vector and infection history matrix for a specific gravidity category
...
get_weight_impact_df <- function(inf_history, primi_prev, imm_vect) {
  # Find the index that matches the primi_prev closest to the infection history
  ...
  effects <- c(1,  # Gravidity category 1 is the baseline (no previous exposure)
