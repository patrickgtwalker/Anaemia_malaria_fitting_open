...
At first glance this may seem unintuitively high given prevalence was 10% at first ANC in their previous pregnancy. In part this is because the model capture cumulative prevalence from 12 weeks gestation (when the placenta is considered sufficiently viable to lead to substantial sequestration) to delivery but is largely due to heterongeneity in exposure within the community - women who are infected last pregnancy are more likely than average to be infected this one. 

To model how prior exposure mitigates the malaria-associated impact on hemoglobin, we use a sigmoidal function, which is commonly used in biological contexts to describe saturation effects.
```{r Sigmoid_func, echo = TRUE,fig.width=5}

# Example usage of the sigmoidal function
shape <- 2
scale <- 1
immunity_vector <- Sigmoid_func(0:20, shape, scale)
...
ggplot(data.frame(i = 0:20, immunity = immunity_vector), aes(x = i, y = immunity)) +
  geom_line() +
  labs(title = "Immunity Vector Modeled by Sigmoidal Function",
...
imm_vect <- Sigmoid_func(0:20, shape = 2, scale = 3)
...
knitr::kable(head(synthetic_data$data_df))

### plot the dataset
ggplot(synthetic_data$data_df)+
geom_point( aes(x = gestage, y = hb_level, color = factor(malaria)), alpha = 0.05) +
  stat_smooth(data = synthetic_data$data_df, aes(x = gestage, y = hb_level, color = factor(malaria)), method = "loess", se = FALSE, linetype = "dotted", size = 2) +
